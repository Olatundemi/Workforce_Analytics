...
def preprocess_and_prepare_data():
    ...
    if current_data and leavers_data:
        ...
        if columns_to_drop:
            ...
            cleaned_data = handle_missing_values(cleaned_data)  # Call outside cache
            st.write("Dataset with Selected Columns Dropped and Missing Values Handled")
            st.write(cleaned_data)
        elif not columns_to_drop and missing_values:
            cleaned_data = handle_missing_values(cleaned_data)  # Call outside cache
        ...
        return X, y, cleaned_data, all_columns_dict  # Return cleaned_data and all_columns_dict
    else:
        st.error("Upload Dataset to Get Started")
        return None, None, None, None

# Define a separate function for handling missing values (without caching)
#def handle_missing_values(data):
    # Allow the user to choose how to handle missing values
 #   option = st.radio("Choose an option to handle missing values:", ('Fill data', 'Drop Null Values'))

  #  if option == 'Fill data':
   #     fill_value = st.text_input("Enter the value to fill missing data:")
    ...
    if X is not None and y is not None:
        ...
        if nulls_X or nulls_y > 0:
            ...
            if st.button('Train Model'):
                ...
                # Train the model
                _best_model = train_model(X_train_resampled, y_train_resampled, X_test, y_test)
                ...
                return _best_model  # Return the trained model
    else:
        st.error("Please upload and preprocess the data first.")

@st.cache_data
def make_predictions(_best_model, total_current_staffs_features):
    # Making predictions
    total_current_staffs_predictions = _best_model.predict(total_current_staffs_features)
    return total_current_staffs_predictions

# Main app
st.title("HR Analytics App")

# Step 1: Preprocess and prepare data (X, y, cleaned_data, and all_columns_dict)
X, y, cleaned_data, all_columns_dict = preprocess_and_prepare_data()

# Step 2: Train the model (only if data is prepared)
if X is not None and y is not None:
    # Data Splitting
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=2)
    ...
    if _best_model is not None:
        ...
        # Cache and make predictions
        total_current_staffs_predictions = make_predictions(_best_model, total_current_staffs_features)
        ...
        no_attrition = cleaned_data.drop('Attrition', axis=1)  # Now cleaned_data is available
        columns_to_reverse_encode = no_attrition.columns
        sorted_total_current_staffs_decoded = reverse_encode_columns(
            sorted_total_current_staffs, 
            {col: all_columns_dict.get(col, {}) for col in columns_to_reverse_encode}
        )
        ...
        st.error("Model training failed. Please check the data.")
